...
still streaming through it. A streaming response holds the connection it is
reading from but does not keep the handler alive. The handler is
...
def _select_transport(monkeypatch, disable_aiohttp_transport: bool) -> None:
    # The parent fixture reloads litellm (and dotenv), so isolate proxies here.
...
async def test_async_stream_survives_handler_collection(monkeypatch, disable_aiohttp_transport):
    ...
    with _ChunkedSSEServer() as server:
        ...
        handler.client._mounts.clear()  # Exclude macOS system proxy mounts too.
        ...
        await asyncio.sleep(0)
        await asyncio.sleep(0)
        await _wait_until(lambda: ref() is None, NOT_RELEASED)
...
def test_sync_stream_survives_handler_collection(monkeypatch):
    ...
    """
    _select_transport(monkeypatch, True)
...
async def test_a_non_streaming_response_does_not_pin_its_handler(monkeypatch, disable_aiohttp_transport):
    ...
    with _ChunkedSSEServer(frame_count=1, frame_delay=0.0) as server:
        ...
        await _wait_until(lambda: ref() is None, "a fully-read response pinned its handler")
